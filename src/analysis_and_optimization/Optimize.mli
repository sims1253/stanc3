...
(** Inline all functions except for ones with forward declarations (e.g.
    recursive functions, mutually recursive functions, and functions without a
    definition *)

val static_loop_unrolling : Program.Typed.t -> Program.Typed.t
(** Unroll all for-loops with constant bounds, as long as they do not contain
    break or continue statements in their body at the top level *)
...
val expression_propagation :
  ?preserve_stability:bool -> Program.Typed.t -> Program.Typed.t
(** Propagate arbitrary expressions through variable assignments. This can be
    useful for opening up new possibilities for partial evaluation. It should be
    followed by some CSE or lazy code motion pass, however. *)
...
(** Eliminate semantically redundant code branches. This includes removing
    redundant assignments (because they will be overwritten) and removing
    redundant code in program branches that will never be reached. *)

val partial_evaluation : Program.Typed.t -> Program.Typed.t
(** Partially evaluate expressions in the program. This includes simplification
    using algebraic identities of logical and arithmetic operators as well as
    Stan math functions. *)

val lazy_code_motion :
  ?preserve_stability:bool -> Program.Typed.t -> Program.Typed.t
(** Perform partial redundancy elmination using the lazy code motion algorithm.
    This subsumes common subexpression elimination and loop-invariant code
    motion. *)
...
(** Marks Decl types such that, if the first assignment after the decl assigns
    to the full object, allow the object to be constructed but not
    uninitialized. *)
