...
(* The gathered-GLM primitive (W-108): only included when
   [Optimize.gather_bernoulli_logit] rewrote a likelihood in this program, so
   pattern-free models keep their exact include list. *)
let gathered_bernoulli_include =
  Preprocessor (Include "stan/math/rev/prob/bernoulli_logit_lpmf_gathered.hpp")

let uses_gathered_bernoulli_logit (p : Program.Typed.t) =
  let found = ref false in
  let rec scan_expr (e : Expr.Typed.t) =
    (match e.pattern with
    | FunApp (StanLib ("bernoulli_logit_lpmf_gathered", _, _), _) ->
        found := true
    | _ -> ());
    if not !found then ignore (Expr.Pattern.map scan_expr e.pattern) in
...
let lower_program ?(standalone_functions = false) ?printed_filename
  ...
  (* First, do some transformations on the MIR itself before we begin printing
     it.*)
  let uses_gathered = uses_gathered_bernoulli_logit p in
  ...
  let includes =
    if uses_gathered then includes @ [gathered_bernoulli_include]
    else includes in
  let global_fns =
