...
(* The gathered-families registry (W-108 increment 2 + W-115): the header of
   every family whose primitive call the optimizer actually emitted in this
   program is included, right after [model_header.hpp] (the include position
   of the gated hand-edits), so pattern-free models keep their exact include
   list. *)
let gathered_includes (p : Program.Typed.t) : Cpp.defn list =
  let found = ref String.Set.empty in
  let wanted = List.length Gathered_Families.primitives in
  let rec scan_expr (e : Expr.Typed.t) =
    (match e.pattern with
    | FunApp (StanLib (name, _, _), _)
      when List.mem ~set:Gathered_Families.primitives name ->
        found := String.Set.add name !found
    | _ -> ());
    if String.Set.cardinal !found < wanted then
      ignore (Expr.Pattern.map scan_expr e.pattern) in
  let rec scan_stmt (s : Stmt.Located.t) =
    if String.Set.cardinal !found < wanted then
      ignore (Stmt.Pattern.map scan_expr scan_stmt s.pattern) in
  List.iter ~f:scan_stmt p.reverse_mode_log_prob;
  List.filter_map Gathered_Families.families
    ~f:(fun (fam : Gathered_Families.t) ->
      if String.Set.mem fam.primitive !found then
        Some (Preprocessor (Include fam.header))
      else None)
...
let lower_program ?(standalone_functions = false) ?printed_filename
  ...
  (* First, do some transformations on the MIR itself before we begin printing
     it.*)
  let gathered = gathered_includes p in
  ...
  let includes = includes @ gathered in
  let global_fns =
